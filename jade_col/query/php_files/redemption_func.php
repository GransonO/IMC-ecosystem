...
if($_POST['process_code']== 1){
	
	$member_id = $_POST['reward_id'];
	$get_Points = "SELECT `total_points_available` AS points, `company_name` AS company, `redemptions_count` AS count, `total_points_redeemed` AS red FROM `member_loyalty_funds` WHERE `member_id` = '$member_id'"; 
	$points_query =  mysqli_fetch_assoc(mysqli_query(Connect_Database(),$get_Points));
	if($points_query['company']){
			$points_result = $points_query['points']; 
			$Company_name = trim($points_query['company']);
			
			$myObj = new \stdClass();
			$myObj->name = $Company_name;
			$myObj->points = $points_result;
...
if($_POST['process_code']== 2){
	$member_id = $_POST['reward_id'];
	$reward_request = $_POST['reward_request'];
	...
	$The_Company = $points_query['company'];
		...
		if ($deal == 1 && $real == 1) {
				...
				redemption_mailer($member_id,$redeem_date,$username,$reward_request);
...
function redemption_mailer($member_id,$date_o_red,$redeemer,$points_redeemed){
				$mail_message_html =
								'
<!doctype html>
	<html lang="en">
								...
								<div class="wizard-header">
									<h3>
									   ...
									   <div class="wizard-footer">	
												<h3>
													<div style="text-align:center;">
														<img src="https://indulgecard.co.ke/interpel/interpelrewards/production/images/logo.png"  height="100" width="200">
													</div>
												</h3>
								</div>
