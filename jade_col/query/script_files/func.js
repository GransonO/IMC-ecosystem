...
$('button#send').on('click',function(){
	...
	var message_body = $('textarea#text').val();
	var message_title = $('input#title').val();
	var message_recipients = $('select#customer_type').val();
	...
	if($('input#check').is(':checked')){
 	...
 	if(($.trim(message_body) != '')&&($.trim(message_title) != '')&&($.trim(message_recipients) != '')){
		swal({
			  title: "Sending message...",
			  text: "Confirm to send the message to recipients.",
			  type: "info",
			  confirmButtonColor: "#1dbf07",
			  confirmButtonText: "Yes, Go On!",
			  showCancelButton: true,
			  ...
			  showLoaderOnConfirm: true,
					...
					if (isConfirm) {
					 Send_Message(message_body,message_title,message_recipients,personalize);
					}
		});
	}else{
		swal({
			title:"Sending error!",
			text:"Please ensure the Message, title $ recipients are all indicated.",
			type: "error",
			timer: 2500,
			showConfirmButton: false
			});
...
//Sending Message to rercipients
function Send_Message(message_body,message_title,message_recipients,personalize){
 	
		$.post('../../query/php_files/send_messages.php', 
		{message_body:message_body,message_title:message_title,message_recipients:message_recipients,personalize:personalize},
		function(data){
			alert(data);
				if(data == 2){
