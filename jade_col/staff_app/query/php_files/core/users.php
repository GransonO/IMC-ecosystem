...
function user_priviledge($username){
	$username = sanitize($username);
	$query = mysqli_query(Connect_Database(),"SELECT COUNT(`username`) AS count FROM `privileged_users` WHERE `username`= '$username'");
	$result = (mysqli_fetch_assoc($query));
	$food = $result["count"];
...
function priviledged_login($username,$password){
