...
if((isset($_POST['recover_pass_name'])===true && empty($_POST['recover_pass_name'])=== false) &&
  (isset($_POST['recover_pass_email'])===true && empty($_POST['recover_pass_email'])=== false)){
	...
	if(user_exists($username) < 1){
		...
		$profile_query = mysqli_query(Connect_Database(),"INSERT INTO `nkl_password_recovery` (`date`, `username`, `email`) VALUES ('$insert_date', '$username', '$email')");
...
function recovery_mail($username,$email,$generated_password){
			...
			$message_subject = 'Naivasha Kongoni Lodge Loyalty Program Password Recovery.';
			...
			$message = (new Swift_Message($message_subject))
			  ->setFrom(['kongoniloyalty@indulgencemarketing.co.ke' => 'Kongoni Loyalty Program Password Recovery'])
			  ->setTo($email)
